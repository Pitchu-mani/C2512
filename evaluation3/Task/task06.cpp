...
void LabTestManager::deleteById()
{
    ...
    // Remove the LabTest from the vector
...
// Replaced Fixed-Size Array with std::vector:

// LabTests[MAX_TESTS] → std::vector<LabTest> LabTests.
// Dynamic Operations:

// push_back to add new items.
// erase to remove an item.
// Removed numTests Attribute:

// // The size of the vector is dynamically managed and can be accessed with LabTests.size().
// // Adjusted Functions:
//  Advantages of size_t
// Prevents warnings or errors when comparing sizes or indices because of mismatched types.
// Iterations and size checks now use vector operations.
