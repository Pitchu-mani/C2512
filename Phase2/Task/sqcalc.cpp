...
void serveClient(int &client_socket_fd)
{
    ...
    // process  square of input number
    long sqr = num + second;
    ...
    // send response
    std::string sqrStr = std::to_string(sqr);
    send(client_socket_fd, sqrStr.c_str(), sqrStr.size() + 1, 0);
...
void requestServer(int &client_socket_fd)
{
    ...
    send(client_socket_fd, numStr.c_str(), numStr.size() + 1, 0);

    // receive response
    char buffer[BUFFER_SIZE];
    read(client_socket_fd, buffer, BUFFER_SIZE);
    //
    long sqr = atol(buffer);
    std::cout << "So," << num << " * "
              << num << " = "
              << sqr << std::endl;
    std::cout << "I am thankful to my square server!!!" << std::endl;
}
